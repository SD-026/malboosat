...
function Myorder() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get(`http://localhost:5555/api/myorder`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        console.log("sheeraz",response.data.existingProduct)
        ...
        {orders.map((order) => (
          <div key={order._id} className="bg-white rounded-lg shadow-lg p-6 hover:scale-105 transition-transform duration-300">
            ...
            <div className="flex justify-between items-center">
              ...
              <button onClick={(order)=>cancelOrder(order)} className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-700">
                Cancel Order
              </button>
            </div>
          </div>
