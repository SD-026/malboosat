...
const S_SingleProduct = () => {
  ...
  const AddtoCart = async() => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:5555/api/addtocart`,predata,{
        headers: {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`http://localhost:5555/api/getproduct/${id}`);
        setData(response.data.singleproduct); // Assuming single product comes in the "singleproduct" field
        setLoading(false); // Turn off loading
      } catch (error) {
        console.error(error);
  ...
  const P_Delete=async()=>{
      ...
      try {
      const response = await axios.put(`http://localhost:5555/seller/remove_Seller_item`,predata,{
        headers: {
         Authorization: `Bearer ${token}`,
     },
    });
  ...
  return (
    <div className="bg-white">
      ...
      <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        ...
        <button 
              className="mt-4 text-sm w-auto p-3 font-semibold py-2 rounded-3xl bg-red-500"
              onClick={P_Delete}
            >
              DElete product 
            </button>
...
export default S_SingleProduct;
